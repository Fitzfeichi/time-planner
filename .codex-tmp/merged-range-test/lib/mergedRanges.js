function createRangeId(startSlotId, endSlotId) {
...
function getSlotIndexById(slots) {
    return new Map(slots.map((slot, index) => [slot.id, index]));
}
function getSortedUniqueSlotIndexes(slots, slotIds) {
...
export function getMergedRangeForSlot(slots, mergedRanges = [], slotId) {
    return (mergedRanges.find((range) => getMergedRangeSlotIds(slots, range).includes(slotId)) ?? null);
}
export function canCreateMergedRange(slots, mergedRanges = [], selectedSlotIds) {
...
export function hasMergedRangeConflict(slots, selectedSlotIds, sourceSlotId) {
...
export function applyMergedRange(slots, mergedRanges = [], selectedSlotIds, sourceSlotId) {
    ...
    return {
        didMerge: true,
        slots: slots.map((slot, index) => selectedIndexSet.has(index)
            ? {
                ...slot,
...
export function removeMergedRangeForSlot(mergedRanges = [], slotId) {
